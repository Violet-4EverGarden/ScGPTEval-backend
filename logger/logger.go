...
func Init(cfg *config.LogConfig, mode string) (err error) {
	...
	if mode == "dev" {
		...
		core = zapcore.NewTee(
			zapcore.NewCore(encoder, writeSyncer, l),                                     // 在文件中写
	...
	zap.ReplaceGlobals(lg) // 替换zap库中全局的logger实例，后续在其他包或函数中只需使用zap.L()调用即可
	return
}
...
// 使用第三方库Lumberjack进行日志切割归档（因为zap库本身不支持切割归档日志文件）
